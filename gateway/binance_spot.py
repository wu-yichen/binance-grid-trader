...
class BinanceSpotHttp(object):
        ...
        for i in range(0, self.try_counts):
            try:
                response = requests.request(
                    req_method.value, url=url, headers=headers, timeout=self.timeout)
    ...
    def _sign(self, params):
        ...
        hex_digest = hmac.new(self.secret.encode(
            'utf8'), query_string.encode("utf-8"), hashlib.sha256).hexdigest()
    ...
    def cancel_order(self, symbol, client_order_id):
        ...
        for i in range(0, 3):
            try:
                order = self.request(RequestMethod.DELETE,
                                     path, params, verify=True)
                return order
            except Exception as error:
